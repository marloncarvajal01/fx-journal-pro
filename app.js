...
async function register() {
    const email = document.getElementById('login-email').value;
    const pass = document.getElementById('login-pass').value;
    try { await auth.createUserWithEmailAndPassword(email, pass); alert("Cuenta creada"); } catch(e) { alert(e.message); }
}
...
async function loadCloudData() {
    ...
    .orderBy('date', 'asc').onSnapshot(snap => {
        ...
        navigate('newTrade'); // AL ENTRAR, VA DIRECTO A NUEVO TRADE
    });
}
...
function renderNewTrade(editId = null) {
  ...
  document.getElementById("content").innerHTML = `
    <h1>${editId !== null ? "Editar" : "Nuevo"} Trade</h1>
    <div class="card">
      <div class="input-row">
      ...
      <div class="checklist-group">
        <h4 style="margin-top:0; color:#22c55e; margin-bottom: 15px;">🔥 Confirmaciones</h4>
      ...
      <button onclick="saveToCloud('${editId || ""}')">Guardar en la Nube</button>
    </div>
  `;
}
...
async function saveToCloud(editId) {
  ...
  const data = {
    ...
    rrRatio: (rType === "win" && riskVal > 0) ? (rVal / riskVal).toFixed(2) : (rType === "loss" ? -1 : 0),
...
function renderDashboard() {
  ...
  document.getElementById("content").innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 25px;">
        ...
        <button onclick="exportData()" style="width:auto; background:#3b82f6; display:flex; align-items:center; gap:8px;">📥 Exportar a Excel</button>
    </div>
    ...
    <div class="stats-container" style="margin-top: 0; margin-bottom: 25px;">
      ...
      <div class="stat-card"><h3>Drawdown Actual</h3><span class="value red">-$${drawdown.toFixed(2)}</span><p style="font-size:13px; margin-top:10px;">${drawdownPercent}% desde el pico</p></div>
    </div>
    <div class="card"><canvas id="equityChart"></canvas></div>
  `;
  if (equityChart) equityChart.destroy();
  equityChart = new Chart(document.getElementById("equityChart"), { 
      type: "line", data: { labels: ["Inicio", ...trades.map((_, i) => "T" + (i + 1))], datasets: [{ label: "Capital", data: equityData, borderColor: "#22c55e", fill: true, backgroundColor: "rgba(34,197,94,0.1)" }] } 
  });
}
...
async function deleteFromCloud(id) { 
    if(confirm("¿Borrar?")) await db.collection('trades').doc(currentUser.uid).collection('userTrades').doc(id).delete(); 
}
...
function exportData() {
    let csv = "data:text/csv;charset=utf-8,Fecha,Par,Resultado,Error\n";
    trades.forEach(t => csv += `${t.date},${t.pair},${t.result},${t.errorTag}\n`);
